...
public class GourmetCoffee  {
	...
	public void displayNumberOfOrders(Product product) {
		...
		for (Order temp_order : sales) {
			for (OrderItem temp_item : temp_order) {
				if (product.equals(temp_item.getProduct())) {
					NumberOfOrders += 1;
				}
			}
		}
		System.out.println("Number of orders that contains the product "+product.getCode()+" "+NumberOfOrders);
	}
	...
	public void displayTotalQuantityOfProducts() {
		...
		for (Product product:catalog) {
			temp_product_code=product.getCode();
			for (Order temp_order:sales) {
				for (OrderItem temp_item:temp_order) {
					if (temp_item.getProduct().getCode()==temp_product_code){
						Total_sold+=temp_item.getQuantity();
					}
				}
			}
			System.out.println(product.getCode()+" "+Total_sold);
			Total_sold = 0;
		}
